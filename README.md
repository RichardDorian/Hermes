...
| `WAN_ADDRESS`                  |                 | Required. Address of `WAN_IF` in CIDR notation (e.g. `192.168.1.10/24`), used as the NAT source. Floated with `LAN_ADDRESS` when `ENABLE_KEEPALIVED=true`, assigned statically otherwise. |
...
the virtual network a highly available gateway/DHCP/DNS server. `WAN_ADDRESS` is floated
on `WAN_IF` in the same VRRP instance as `LAN_ADDRESS`, so both always move to the same node, and NAT sources
outgoing traffic from it instead of each node's own WAN address. Every node starts with the same priority, so
which one currently holds the address is decided automatically by VRRP rather than configured per node. VRRP
advertisements are unauthenticated, so only enable this on a LAN you trust.
